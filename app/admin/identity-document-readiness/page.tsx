...
export default function IdentityDocumentReadinessPage() {
  const report = getIdentityDocumentReadinessReport()
      ...
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-soft">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-black text-emerald-800"><IdCard className="h-5 w-5" /> Phase 108</div>
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Identity document correction readiness</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Readiness controls for Aadhaar, PAN, voter ID, passport, driving license, certificates and bank KYC correction planning.</p>
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>API: /api/admin/identity-document-readiness · Evidence: npm run identity-document:readiness</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Review before enabling this identity correction workflow at scale.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2">
              {report.identityDocumentReadinessLanes.map((lane) => (
