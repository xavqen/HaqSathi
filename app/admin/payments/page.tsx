...
export default async function Page() {
  ...
  return <div><h1 className="text-3xl font-black">Payments</h1><p className="mt-2 text-slate-600">Razorpay-ready orders and webhook status.</p><a href="/admin/payment-lifecycle" className="mt-4 inline-flex min-h-11 items-center rounded-xl bg-primary px-4 py-2 text-sm font-black text-primary-foreground">Open payment lifecycle QA</a><Card className="mt-6"><CardHeader><CardTitle>Latest payment orders</CardTitle></CardHeader><CardContent className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="py-2">User</th><th>Plan</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>{payments.map(p => <tr key={p.id} className="border-b"><td className="py-3">{p.user?.email || 'guest'}</td><td>{p.plan}</td><td>₹{p.amount / 100}</td><td>{p.status}</td><td>{p.createdAt.toDateString()}</td></tr>)}</tbody></table>{payments.length === 0 && <p className="text-slate-500">No payment orders yet.</p>}</CardContent></Card></div>
}
