...
export default function VehicleChallanReadinessPage() {
  const report = getVehicleChallanReadinessReport()
      ...
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-soft">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-black text-emerald-800"><Car className="h-5 w-5" /> Phase 107</div>
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">Vehicle challan dispute readiness</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Readiness controls for wrong challan, paid-but-pending challan, duplicate challan, towing/seizure and vehicle-sold dispute planning.</p>
            </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>API: /api/admin/vehicle-challan-readiness · Evidence: npm run vehicle-challan:readiness</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Review before enabling this transport workflow at scale.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2">
              {report.vehicleChallanReadinessLanes.map((lane) => (
