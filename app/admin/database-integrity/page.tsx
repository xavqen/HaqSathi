import { AlertTriangle, CheckCircle2, Database, FileCheck2, KeyRound, ListChecks, LockKeyhole, RefreshCcw, ServerCog, ShieldCheck } from 'lucide-react'
import { requireAdmin } from '@/lib/auth/session'
import { getDatabaseIntegrityReadinessReport, type DatabaseIntegrityPriority, type DatabaseIntegrityStatus } from '@/lib/database/integrity-readiness'
import { Badge } from '@/components/ui/badge'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'

function statusClass(status: DatabaseIntegrityStatus) {
...
function priorityClass(priority: DatabaseIntegrityPriority) {
...
export default async function DatabaseIntegrityPage() {
  await requireAdmin()
  const report = getDatabaseIntegrityReadinessReport()
    ...
    <div className="grid gap-6">
      <section className="rounded-[2rem] border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-5 shadow-soft sm:p-7">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <Badge>Phase 76</Badge>
            <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Database integrity readiness</h1>
            <p className="mt-3 text-sm leading-6 text-slate-700 sm:text-base">
              Launch guard for Prisma schema validation, production database URLs, migrations, seed idempotency, indexes, backups, pooling and Supabase policy review.
            </p>
          </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <Card>
          <CardHeader>
            ...
            <CardTitle>Database launch controls</CardTitle>
            <CardDescription>Keep repair/destructive operations manual until migration, backup, seed and policy evidence is saved.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="grid gap-6">
          <Card>
            ...
            <CardContent>
              <div className="rounded-2xl bg-slate-950 p-4 font-mono text-xs font-bold leading-6 text-emerald-100">
                npm run database:integrity-readiness<br />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Use this before real public users and payment data enter the database.</CardDescription>
            </CardHeader>
            <CardContent>
              <ul className="grid gap-2 text-sm leading-6 text-slate-700">
                <li className="flex gap-2"><KeyRound className="mt-1 h-4 w-4 shrink-0 text-emerald-700" />Verify DATABASE_URL and DIRECT_URL are real production/staging secrets.</li>
                <li className="flex gap-2"><RefreshCcw className="mt-1 h-4 w-4 shrink-0 text-emerald-700" />Take a backup before migration/db push and prove restore on staging.</li>
                <li className="flex gap-2"><AlertTriangle className="mt-1 h-4 w-4 shrink-0 text-rose-600" />Never run destructive repair/seed operations on production without manual review.</li>
              </ul>
            </CardContent>
          </Card>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardTitle>Critical dataset map</CardTitle>
          <CardDescription>Models and integrity checks that need evidence before public launch.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {report.datasets.map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                ...
                <p className="mt-2 break-words rounded-xl bg-slate-50 p-3 font-mono text-xs font-semibold leading-6 text-slate-800">{item.models.join(', ')}</p>
                <ul className="mt-3 grid gap-1.5 text-sm text-slate-700">
                  {item.integrityChecks.map((check) => <li key={check} className="flex gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700" />{check}</li>)}
                </ul>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Use this before schema, seed, backup, auth, vault or payment changes.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="grid gap-2 text-sm text-slate-700">
              <li className="flex gap-2"><Database className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700" />Database integrity JSON/CSV from local evidence generator.</li>
              <li className="flex gap-2"><FileCheck2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700" />Prisma validate/generate and staging migration screenshots.</li>
              <li className="flex gap-2"><RefreshCcw className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700" />Backup restore drill evidence with recovered critical rows.</li>
              <li className="flex gap-2"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" />RLS/storage policy and user-owned record access test proof.</li>
            </ul>
          </CardContent>
        </Card>
      </div>
