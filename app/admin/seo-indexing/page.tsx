import { AlertTriangle, CheckCircle2, ExternalLink, FileSearch, Globe2, ListChecks, SearchCheck } from 'lucide-react'
...
export const metadata = { title: 'SEO Indexing Readiness | Admin | HaqSathi AI' }
...
export default async function AdminSeoIndexingPage() {
  await requireAdmin()
  const report = getSeoIndexingReadinessReport()
  const cards = [
    { label: 'Controls', value: report.summary.totalControls, note: `${report.summary.ready} ready now`, icon: ListChecks },
    { label: 'Manual gates', value: report.summary.manualRequired, note: 'Need real Search Console proof', icon: AlertTriangle },
    { label: 'P0 SEO lanes', value: report.summary.p0Lanes, note: 'Must verify before traffic', icon: SearchCheck },
    { label: 'Core routes', value: report.summary.indexedRoutes, note: 'Configured for indexing review', icon: Globe2 }
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-5 shadow-soft sm:p-7">
        <Badge className="border-emerald-200 bg-emerald-100 text-emerald-800">Phase 66</Badge>
        <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="min-w-0">
            <h1 className="text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">SEO indexing readiness</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 sm:text-base">Production Search Console, sitemap, robots, canonical and indexing evidence gate before public SEO traffic. This keeps localhost checks separate from real deployed-domain proof.</p>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <a href="/api/admin/seo-indexing-readiness" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border bg-white px-4 py-2 text-sm font-black text-slate-700 shadow-sm"><ExternalLink className="h-4 w-4" />Open API</a>
            <a href="/admin/seo-audit" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2 text-sm font-black text-primary-foreground shadow-sm"><FileSearch className="h-4 w-4" />SEO audit</a>
          </div>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardTitle>Production indexing controls</CardTitle>
          <CardDescription>These checks should be completed on the deployed domain, not only local development.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <SearchCheck className="h-6 w-6 text-emerald-700" />
          <CardTitle>Search Console evidence lanes</CardTitle>
          <CardDescription>Review these lanes before backlinks, ads, social announcements or heavy SEO traffic.</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div key={lane.id} className="rounded-2xl border bg-white p-4">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 break-words text-xs font-bold text-slate-500">Route: {lane.route}</p>
                  </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardTitle>Search Console checklist</CardTitle>
            <CardDescription>Use this after the Vercel production URL is live.</CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="grid gap-3 text-sm text-slate-700">
              {report.searchConsoleChecklist.map((item) => (
        ...
        <Card>
          <CardHeader>
            <CardTitle>Indexing runbook</CardTitle>
            <CardDescription>Save evidence before setting approval env flags to true.</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <pre className="mt-5 overflow-x-auto rounded-xl bg-slate-950 p-3 text-xs font-bold text-emerald-100">npm run seo:indexing-readiness</pre>
          </CardContent>
        </Card>
      </div>
    </div>
