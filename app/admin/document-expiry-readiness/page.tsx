import { CalendarClock, CheckCircle2, FileWarning, ShieldCheck, TerminalSquare } from 'lucide-react'
import { AdminShell } from '@/components/admin/admin-shell'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { getDocumentExpiryReadinessReport, type DocumentExpiryReadinessPriority, type DocumentExpiryReadinessStatus } from '@/lib/documents/document-expiry-readiness'

export const metadata = { title: 'Document Expiry Readiness | Admin' }
export const dynamic = 'force-dynamic'

function statusClass(status: DocumentExpiryReadinessStatus) {
...
function priorityClass(priority: DocumentExpiryReadinessPriority) {
...
export default function DocumentExpiryReadinessAdminPage() {
  const report = getDocumentExpiryReadinessReport()
  return (
    <AdminShell>
      <div className="grid gap-6">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-soft sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-black text-emerald-800"><CalendarClock className="h-5 w-5" /> Phase 90</div>
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">Document expiry planner readiness</h1>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600 sm:text-base">
                Review document renewal reminders, official-only routes, privacy copy and mobile UX before promoting the planner publicly.
              </p>
            </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Official routes, privacy warnings, reminders and mobile QA gates.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <div className="grid gap-6">
            <Card>
              ...
              <CardContent>
                <div className="rounded-2xl bg-slate-950 p-4 font-mono text-xs font-bold leading-6 text-emerald-100">
                  npm run document-expiry:readiness<br />
                  /api/admin/document-expiry-readiness<br />
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Rules for document reminders and renewal guidance.</CardDescription>
              </CardHeader>
              <CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardTitle>Document lanes</CardTitle>
            <CardDescription>Priority reminder lanes and official-only safety rule for each document group.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {report.documentExpiryReadinessLanes.map((lane) => (
                <div key={lane.id} className="rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <h2 className="mt-3 font-black text-slate-950">{lane.document}</h2>
                  <p className="mt-2 text-sm leading-6 text-slate-700"><strong>Window:</strong> {lane.renewalWindow}</p>
                  <p className="mt-2 text-sm leading-6 text-slate-700"><strong>Route:</strong> {lane.officialRoute}</p>
