...
export default async function EvidencePacksPage() {
  ...
  return <div><h1 className="text-4xl font-black">Evidence Packs</h1><p className="mt-2 text-slate-600">Create a cover note to organize proof files in annexure order.</p><div className="mt-8"><EvidencePackForm /></div><Card className="mt-8"><CardHeader><CardTitle>Recent packs</CardTitle></CardHeader><CardContent>{packs.length === 0 ? <p className="text-slate-500">No evidence packs yet.</p> : <div className="space-y-3">{packs.map((pack) => <div key={pack.id} className="rounded-xl border p-4"><b>{pack.caseTitle}</b><p className="text-sm text-slate-600">{pack.category} · {pack.createdAt.toDateString()}</p><pre className="mt-3 max-h-40 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-xs">{pack.coverNote}</pre></div>)}</div>}</CardContent></Card></div>
}
