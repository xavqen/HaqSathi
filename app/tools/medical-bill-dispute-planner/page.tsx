...
export default function MedicalBillDisputePlannerPage() {
  return (
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:py-14">
        <div className="rounded-[2rem] border border-emerald-100 bg-white p-6 shadow-soft sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex items-center gap-2 text-sm font-black text-emerald-800"><Hospital className="h-5 w-5" /> New billing helper</div>
              <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-950 sm:text-5xl">Medical bill dispute planner</h1>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {[
            ['Itemized breakup', 'Ask for written item-wise billing breakup and package/estimate comparison.'],
            ['Insurance/TPA proof', 'Track pre-auth, deduction, denial or reimbursement details safely.'],
            ['Safe escalation', 'Avoid sharing OTP, CVV, UPI PIN, password and unnecessary health details.']
          ].map(([title, desc]) => (
        ...
        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
          ...
          <p className="mt-1">This tool gives billing-dispute guidance only, not medical, legal, insurance or financial advice. For urgent health risk, prioritize qualified medical care.</p>
        </div>

        <div className="mt-6">
          <MedicalBillDisputePlannerForm />
        </div>
      </section>
    </main>
  )
