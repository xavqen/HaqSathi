...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-slate-50"><section className="mx-auto max-w-4xl px-4 py-12"><p className="text-sm font-bold text-emerald-700">{template.category}</p><h1 className="mt-2 text-4xl font-black">{template.title}</h1><p className="mt-3 text-slate-600">{template.intent}</p><Card className="mt-8"><CardHeader><CardTitle>Fill details</CardTitle></CardHeader><CardContent><TemplateRenderForm slug={template.slug} variables={variables} isPremium={template.isPremium} /></CardContent></Card><div className="mt-6"><FeedbackForm entity="Template" entityId={template.id || template.slug} /></div><div className="mt-8 rounded-2xl bg-amber-50 p-4 text-sm text-amber-900">Disclaimer: Ye ready draft guidance ke liye hai, official/legal guarantee nahi.</div></section></main>
}
