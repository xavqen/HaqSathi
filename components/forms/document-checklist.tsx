...
export function DocumentChecklistForm() {
  ...
  return <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Checklist details</CardTitle><CardDescription>Choose the document type.</CardDescription></CardHeader><CardContent><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
  ...
  </form></CardContent></Card><Card><CardHeader><CardTitle>Checklist</CardTitle><CardDescription>Verify state rules before final action.</CardDescription></CardHeader><CardContent>{!result ? <p className="rounded-2xl border border-dashed p-8 text-center text-slate-500">Output will appear here.</p> : <div className="space-y-5"><List title="Required documents" items={result.requiredDocuments}/><List title="Optional documents" items={result.optionalDocuments}/><List title="Process" items={result.stepByStepProcess}/><List title="Common mistakes" items={result.commonMistakes}/><p className="rounded-2xl bg-slate-50 p-4 text-sm"><b>Time:</b> {result.timeEstimate}<br/><b>Mode:</b> {result.mode}</p><p className="rounded-2xl bg-amber-50 p-4 text-sm">{result.disclaimer}</p></div>}</CardContent></Card></div>
}
