...
export function SchemeFinder() {
  ...
  return <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Scheme profile</CardTitle><CardDescription>Details add karo, possible schemes + docs milenge.</CardDescription></CardHeader><CardContent><form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
  ...
  </form></CardContent></Card><Card><CardHeader><CardTitle>Scheme suggestions</CardTitle><CardDescription>Official portal verify zaroor karein.</CardDescription></CardHeader><CardContent>{!result ? <p className="rounded-2xl border border-dashed p-8 text-center text-slate-500">Output yahan aayega.</p> : <div className="space-y-5"><div className="space-y-3">{result.possibleSchemes.map(s => <div className="rounded-2xl border p-4" key={s.name}><b>{s.name}</b><p className="mt-2 text-sm text-slate-600">{s.whyMayFit}</p><p className="mt-2 text-xs text-amber-700">{s.caution}</p></div>)}</div><List title="Eligibility clues" items={result.eligibilityExplanation}/><List title="Documents" items={result.requiredDocuments}/><List title="Apply steps" items={result.applySteps}/><p className="rounded-2xl bg-amber-50 p-4 text-sm">{result.officialLinkNote}<br/>{result.disclaimer}</p></div>}</CardContent></Card></div>
}
