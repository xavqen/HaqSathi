...
export function LanguageSwitcher({
  ...
  useEffect(() => {
    ...
    const normalizedCookieLanguage = normalizeLanguageCode(
      cookieLanguage ? decodeURIComponent(cookieLanguage) : current,
    )
  ...
  const currentLabel =
    LANGUAGE_OPTIONS.find((language) => language.code === value)?.label || 'English'
  ...
  return (
    <div ref={rootRef} className="hs-popover-root relative shrink-0">
      <button
        ...
        aria-label={label}
        className="flex h-10 min-w-10 cursor-pointer list-none items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-2 text-xs font-black text-slate-700 shadow-sm transition-colors hover:bg-slate-50 sm:h-11 sm:min-w-0 sm:justify-start sm:px-3"
      >
      ...
      {open && (
        <div className="hs-popover hs-language-popover absolute right-0 z-[75] mt-3 origin-top-right rounded-3xl border border-slate-200 bg-white p-3 shadow-2xl ring-1 ring-slate-900/5">
            ...
            {quickLanguages.map((code) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-12 rounded-2xl border px-3 py-2 text-left text-xs font-black transition-colors ${
                    active
          ...
          <select
            ...
            className="mt-3 h-11 w-full rounded-2xl border border-slate-200 bg-white px-3 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500"
          >
          ...
          <p className="mt-2 px-2 text-[11px] leading-5 text-slate-500">
            This changes the app shell language instantly. Logged-in users can save assistant tone in Profile → Language.
          </p>
        </div>
      )}
    </div>
